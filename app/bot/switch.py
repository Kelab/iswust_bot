...
async def switch(session: CommandSession):
    ...
    parser.add_argument("-r", "--repeat-message", action="store_true", default=False)
    ...
    event = session.event
    event["message"] = msg
    event["to_me"] = True  # ensure to_me
    asyncio.ensure_future(handle_message(session.bot, event))
