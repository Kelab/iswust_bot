...
def get_api(sess: requests.sessions.Session):
    """
    ...
    course_table = parse_course_table(res.text)

    exp_course_table = parse_exp_course_table(sess)
...
def parse_exp_course_table(sess: requests.sessions.Session):
    ...
    # 获取访问链接进行身份验证
    verify = sess.get(API.syk_verify_url)
    ...
    for tr in tbody_1:
        if tr.name == "tr":
            ...
            if course_info_lists[0] != "课程名称":
                # 包含所有课程的list
                courses.append(exp_list_to_dict(course_info_lists))
